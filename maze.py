...
class Maze(Tk.Canvas):
    ...
    def __init__(self, frame):
        self._frame = frame
        self._cells = [[Cell(x, y) for y in range(YCELLS)] \
                       for x in range(XCELLS)]
        for x in range(XCELLS-1):
            for y in range(YCELLS):
                self._link(self._cells[x][y], 'east', self._cells[x+1][y])
        for x in range(XCELLS):
            for y in range(YCELLS-1):
    ...
    def prompt_build(self):
        ...
        factor = input("Enter loop factor (0: No loops; 100: All loops): ")
    ...
    def _plot_walls(self, cell):
        ...
        wallCoords = [(corners[i], corners[(i + 1) % 4]) for i in range(4)]
